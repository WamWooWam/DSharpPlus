    ...
    public class WebSocketClient : IWebSocketClient
    {
        ...
        public async Task DisconnectAsync()
        {
            ...
            try
            {
                ...
                await (this._ws?.CloseOutputAsync(WebSocketCloseStatus.NormalClosure, "", CancellationToken.None) ?? Task.CompletedTask).ConfigureAwait(false);
                await (this._receiverTask ?? Task.CompletedTask).ConfigureAwait(false); // Ensure that receving completed
            }
            catch { }
            finally
            {
